...
function square({kind='card', label='', title=[], lines=[], footer='deutsch-meister.de', accent=null}){
  ...
  const tSizePre = title.length>2 ? 78 : 92;
  let blockH = title.length*(tSizePre+16) + 40 + 74;
  lines.forEach(l => { blockH += l==='' ? 26 : (/^[*#]/.test(l) ? 78 : 66); });
  let y = 300 + Math.max(0, (620 - blockH)/2);
  ...
  lines.forEach(l=>{
    ...
    if (l.startsWith('#')) {
      ...
      out += `<text x="88" y="${y}" font-family="${D}" font-weight="700" font-size="54" fill="${fg}">${esc(a)}</text>`;
      out += `<text x="600" y="${y}" font-family="${D}" font-weight="700" font-size="54" fill="${fg}">${esc(b)}</text>`;
      y += 78; return;
    }
    const strong = l.startsWith('*');
    const txt = strong ? l.slice(1) : l;
    out += `<text x="88" y="${y}" font-family="${strong?D:B}" font-weight="${strong?700:400}" font-size="${strong?54:44}" fill="${strong?fg:sub}">${esc(txt)}</text>`;
    y += strong ? 78 : 66;
  });
...
function thumb({kicker, title=[], level}){
...
sq('t4-akkusativ-01', {kind:'cover', label:'A1 · AKKUSATIV', title:['Nur "der"','ändert sich.'],
  lines:['Der Akkusativ ist einfacher,','als er aussieht.'], accent:AKK});
sq('t4-akkusativ-02', {label:'AKK', title:['Die ganze','Veränderung'],
  lines:['*der → den','*die → die','*das → das','*die (Plural) → die'], accent:AKK});
sq('t4-akkusativ-03', {label:'AKK', title:['Im Satz'],
  lines:['*Ich sehe den Mann.','*Ich sehe die Frau.','*Ich sehe das Kind.','*Ich sehe die Kinder.'], accent:AKK});
sq('t4-akkusativ-04', {label:'MERKE', title:['Die Frage:','Wen oder was?'],
  lines:['Wen sehe ich? → den Mann','','Ein einziger Buchstabe.','Mehr passiert nicht.'], accent:AKK});

/* ===== TAG 5 — sein ==================================================== */
sq('t5-sein-01', {kind:'cover', label:'A1 · VERBEN', title:['sein'],
  lines:['Das wichtigste Verb im','Deutschen — und unregelmäßig.']});
sq('t5-sein-02', {label:'PRÄSENS', title:['ich bin,','du bist, …'],
  lines:['#ich bin|wir sind','#du bist|ihr seid','#er/sie/es ist|sie sind','','Sie sind — die Höflichkeitsform.']});

/* ===== TAG 6 — haben =================================================== */
sq('t6-haben-01', {kind:'cover', label:'A1 · VERBEN', title:['haben'],
  lines:['Das zweite Verb, das du','jeden Tag brauchst.']});
sq('t6-haben-02', {label:'PRÄSENS', title:['ich habe,','du hast, …'],
  lines:['#ich habe|wir haben','#du hast|ihr habt','#er/sie/es hat|sie haben','','Achtung: "du hast" — ohne b.']});

/* ===== TAG 7 — ein oder eine (4 Slides) =============================== */
sq('t7-eineine-01', {kind:'cover', label:'A1 · ARTIKEL', title:['ein oder','eine?'],
  lines:['Wenn du den Artikel kennst,','kennst du auch diesen.']});
sq('t7-eineine-02', {label:'DIE BRÜCKE', title:['Vom der/die/das','zum ein/eine'],
  lines:['*der Tisch → ein Tisch','*die Lampe → eine Lampe','*das Buch → ein Buch']});
sq('t7-eineine-03', {label:'MERKE', title:['Nur "die"','bekommt ein e'],
  lines:['der → ein','die → eine','das → ein','','Ein einziger Buchstabe','unterscheidet die drei.']});
sq('t7-eineine-04', {label:'IM PLURAL', title:['Es gibt kein','"eine" im Plural'],
  lines:['ein Buch → Bücher','','Kein Artikel. Das Wort steht','einfach allein.']});

/* ===== YouTube-Thumbnails ============================================== */
th('yt-t1-thumb', {kicker:'DEUTSCH LERNEN', title:['Das deutsche','Alphabet'], level:'A1'});
th('yt-t2-thumb', {kicker:'AUSSPRACHE', title:['ei, ie, eu, ch'], level:'A1'});
th('yt-t3-thumb', {kicker:'GRAMMATIK', title:['Der Nominativ'], level:'A1'});
th('yt-t4-thumb', {kicker:'GRAMMATIK', title:['Der Akkusativ'], level:'A1'});
th('yt-t5-thumb', {kicker:'VERBEN', title:['Das Verb','sein'], level:'A1'});
th('yt-t6-thumb', {kicker:'VERBEN', title:['Das Verb','haben'], level:'A1'});
th('yt-t7-thumb', {kicker:'ARTIKEL', title:['ein oder eine?'], level:'A1'});
