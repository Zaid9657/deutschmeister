// Admin panel — the cockpit's metric groups, shared by admin-metrics and
// admin-reports so a report can never disagree with the screen it summarises
// (no report introduces a new metric). Each group is pure over the loaders
// below; the endpoint wraps them in safe().
import { rate, fetchAll, exactCount, counting } from './adminHttp.mjs';
import { buildFunnelStages, retentionCohorts, reconcileCoverage, seriesFor } from './adminFunnelLib.mjs';
import { paymentsFromWebhookRows, sumByCurrency, dailySeries, mrrFromSubscriptions } from './adminRevenueLib.mjs';
import { isFailedPayment } from './adminOpsLib.mjs';
import { normalizeLevel, applyLevelFilter, LEVEL_UNKNOWN } from './adminLevels.mjs';
...
export async function revenueGroup(supabase, w) {
...
export async function subsGroup(supabase, now) {
...
export async function activitySets(supabase, level) {
...
export async function usersGroup(supabase, w, now, level, sets) {
...
export async function productGroup(supabase, w, level, sets) {
...
export async function opsGroup(supabase, w) {
