...
| 4 UI reference | ✅ 2026-09-13 | dark `ink` rail (248 px / 72 px collapsed, seeded expanded, state in the shell), white cards on `paper-sunk` with a hairline and no shadows, stat tiles `flex-basis 160` reflowing 4 → 2 → 1 with no media query, headline figures 22/700 and nothing else competing, the four render states kept distinct, a filter bar scoped to `/admin`, `/admin/usage`, `/admin/reports` with `Vergleich: N T davor · Zeitzone UTC` and a freshness button that says *Wird geladen…* / *Aktualisierung fehlgeschlagen*, a mobile icon strip below `lg` (the rail would eat the phone), 16 px side gutter on phones. Verified in Chromium at 390 / 1280 / 1440 px: no horizontal overflow, keyboard focus visible on the first twelve tab stops, no page errors; the viz palette passes the colourblind validator (one contrast warning on `series2`, relieved by axis labels and the data-table toggle) |
