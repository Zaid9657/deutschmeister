...
| O-7 | **Activate the activation-lifecycle mailer** — migration ✅ applied 2026-08-31 via the Supabase connector. Remaining: dry-run (`?dry=1` with `CAMPAIGN_SECRET`; production URL is proxy-blocked from agent sessions, so this is an owner curl) then set `LIFECYCLE_ACTIVATION_ENABLED=true` in Netlify | roadmap §3.1 (Batch E) | 2026-08-22 |
...
| O-15 | **Course go-live prerequisites** (revenue plan Lane 2, code shipped 2026-08-31): apply `migrations/2026-08-31-purchases.sql` (the agent's connector attempt was **blocked by the permission classifier** — either run it in the Supabase SQL editor or approve the tool call); create the one-time LS product "telc B1 Komplettvorbereitung" (€89) in store 309512; enable the `order_refunded` webhook event in the LS dashboard; set `LEMONSQUEEZY_TELC_B1_VARIANT_ID` (Netlify env) + `VITE_LEMONSQUEEZY_TELC_B1_VARIANT_ID` + `PUBLIC_LEMONSQUEEZY_TELC_B1_VARIANT_ID` (the sales page's build-time CTA); create the expiring launch discount code | `docs/revenue-plan-2026-08-31.md`; `migrations/README.md` | 2026-08-31 |
