...
| O-15 | **Course go-live prerequisites** (revenue plan Lane 2): ~~purchases migration~~ ✅ applied 2026-08-31 via the Supabase connector, schema verified. **Remaining (Lemon Squeezy dashboard only — its API cannot create products):** create the one-time product "telc B1 Komplettvorbereitung" (€89) in store 309512; enable the `order_refunded` webhook event; then tell the agent the variant id — it can set the three Netlify env vars (`LEMONSQUEEZY_TELC_B1_VARIANT_ID`, `VITE_…`, `PUBLIC_…`) via the Netlify connector; create the expiring launch discount code | `docs/revenue-plan-2026-08-31.md` | 2026-08-31 |
