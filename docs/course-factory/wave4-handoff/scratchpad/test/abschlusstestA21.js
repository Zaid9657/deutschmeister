...
//   3. playsAllowed — ONE runner change, spelled out below. The listening
//      part carries `playsAllowed: 1`, which today nothing reads.
//      src/pages/Modelltest/ModelltestRun.jsx has a module constant
//      PLAYS_ALLOWED = 2 (line 36) used in exactly two places inside
//      MockListeningPart: the `canPlay` guard (line 82,
//      `const canPlay = plays < PLAYS_ALLOWED;`) and the counter label
//      (line 101, the `Noch ...x abspielbar` template). Line numbers
//      verified against the live file on 2026-09-06 and expected to drift —
//      the constant name is the durable anchor. The integrator
//      adds one line at the top of that component —
//        const playsAllowed = part.playsAllowed ?? PLAYS_ALLOWED;
//      — and points those two existing references at it. Until that lands,
//      this test's Hören section still offers two plays and the section
//      timer cannot honour the second one (see the timing note below), so
//      do NOT ship this module without the runner line. A matching pin
//      belongs in tests/exams.test.mjs next to the selectListeningQuestions
//      pin, for the same reason: a field the runner ignores is a silent
//      no-op, which is exactly how the 23-vs-8 question defect happened.
//
...
// conflict now rests on dependency 3 above.
//
...
// for BOTH rendering and score registration. This exercise carries exactly
// 10 questions today, so the cap is a no-op RIGHT NOW — it is here because
// the A1 exercises also carried 10 until Wave 2/3 expanded every one of them
// to 23, which is precisely how the 23-Hören-vs-8-Lesen weighting defect was
// born. With the cap, the same expansion applied to A2.1 leaves this test at
// its designed 10 Hören : 10 Lesen weighting instead of silently becoming
// 23 : 10. tests/exams.test.mjs pins that every course-test listening part
      ...
      parts: [
        {
          ...
          // fits, two do not. Needs the one-line runner change documented in
          // dependency 3 of this file's header — until then it is inert.
          playsAllowed: 1,
        },
      ],
    },
