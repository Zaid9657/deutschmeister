// Temporary validation harness for the A2.1 Abschlusstest module — mirrors
// the assertions in tests/exams.test.mjs (~226-330) plus the A2.1 level
// constraint (S/wave4/level-a2.1.md), without touching the repo's own test
// file. Run with `node --test`, then delete once the module is integrated
// and the repo suite covers it.
//
// It imports the module straight from the scratchpad (the repo is read-only
// for this author) and the repo's own pure helpers by absolute path.

import { test } from 'node:test';
import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
import { EXAM_TRACKS, examTrackByKey } from '/home/user/deutschmeister/src/data/examTracks.js';
import { countScorableItems } from '/home/user/deutschmeister/src/data/mockExams/index.js';
import { scoreObjectiveSections } from '/home/user/deutschmeister/src/services/examScoring.js';
import { selectListeningQuestions } from '/home/user/deutschmeister/src/data/courseTests/listeningQuestions.js';
import { isLevelFree } from '/home/user/deutschmeister/src/config/freeTier.js';
import { bandCourseForLevel } from '/home/user/deutschmeister/src/data/pricing.js';
import { abschlusstestA21 } from './abschlusstestA21.js';

const S = '/tmp/claude-0/-home-user-deutschmeister/c2792a5e-db22-5773-87d6-2f95d17dddab/scratchpad';
 ...
 */
function headerText() {
  const src = readFileSync(new URL('./abschlusstestA21.js', import.meta.url), 'utf8');
...
test('Hören: exactly one listening part, capped at 10 questions', () => {
  ...
  // The field is inert until the runner reads it. Pin the CURRENT state so
  // this fails the moment ModelltestRun.jsx gains the line — at which point
  // the assertion below flips to the positive form and the note in the
  // module header (dependency 3) can be struck.
  const runSrc = readFileSync('/home/user/deutschmeister/src/pages/Modelltest/ModelltestRun.jsx', 'utf8');
  const runnerReadsIt = /part\.playsAllowed/.test(runSrc);
  assert.equal(
    runnerReadsIt,
    false,
    'ModelltestRun.jsx now reads part.playsAllowed — strike dependency 3 from the header and invert this pin'
  );
  assert.match(runSrc, /const PLAYS_ALLOWED = 2;/, 'the constant this part needs to override still reads 2');
...
test('the chosen listening exercise is the live A2.1 #4 row, verified against the source dump', () => {
  const dump = JSON.parse(readFileSync(`${S}/wave4/source/listening-a2.1.json`, 'utf8'));
  const p = mock.sections.find((s) => s.key === 'hoeren').parts[0];
  const ex = dump.find((e) => e.exercise_number === p.exerciseNumber);
  assert.ok(ex, 'no A2.1 exercise with that exercise_number in the dump');
  assert.equal(ex.id, p.exerciseId, 'exerciseId does not match the live row for that exercise_number');
  assert.equal(ex.title, 'Termine und Verabredungen');
  assert.equal(ex.questions.length, 10, 'the questionMax rationale assumes 10 questions today');
  assert.equal(ex.total_duration_seconds, 518, 'the header quotes 518 s of audio');
  // The header claims two full plays do not fit the 15-minute hard timer.
  assert.ok(ex.total_duration_seconds * 2 > 15 * 60, 'the timing note in the header must stay true');

  // Justification quotes must be real lines of this exercise's transcript.
  const lines = ex.dialogues.flatMap((d) => d.transcript.map((t) => t.text));
  for (const quote of [
    'Samstag um zehn Uhr hätten wir noch einen Termin frei.',
    'Dann könnte unser Techniker morgen zwischen zehn und zwölf Uhr kommen.',
    'Praxis Dr. Weiß, guten Tag.',
    'Hast du am Wochenende Zeit? Wir könnten uns treffen.',
  ]) {
    assert.ok(lines.includes(quote), `header quotes a line not in the transcript: ${quote}`);
  }
  const qTexts = ex.questions.map((q) => q.question_text);
  for (const quote of ['Wann ist der Friseurtermin?', 'Wann kommt der Techniker?', 'Die Frau ist schon Patientin in der Praxis.']) {
    assert.ok(qTexts.includes(quote), `header quotes an item not in the exercise: ${quote}`);
  }

  // And no other mock/course test in the repo already uses an A2.1 exercise.
  for (const f of ['mockExams/goetheA1.js', 'mockExams/goetheB1.js', 'mockExams/telcB1.js', 'mockExams/telcB2.js', 'mockExams/dtz.js', 'courseTests/abschlusstestA11.js', 'courseTests/abschlusstestA12.js']) {
    const body = readFileSync(`/home/user/deutschmeister/src/data/${f}`, 'utf8');
...
test('EVERY double-quoted German string in the header is verbatim source or module text', () => {
  const header = headerText();

  // The dump, flattened: transcripts are stored one paragraph per speaker
  // turn in #4 but one paragraph per whole announcement in #6, so a quoted
  // SENTENCE has to be tested by containment, not list membership. Round 1
  // used membership on four hand-picked quotes and therefore never saw the
  // #6 line that had been stitched together from two announcements.
  const dump = JSON.parse(readFileSync(`${S}/wave4/source/listening-a2.1.json`, 'utf8'));
  const dumpText = flat(
    dump
      .flatMap((e) => [
        e.title,
        ...e.dialogues.flatMap((d) => [d.title, ...d.transcript.map((t) => t.text)]),
        ...e.questions.flatMap((q) => [q.question_text, ...(q.options || [])]),
      ])
      .join(' \n ')
  );
  const moduleText = flat(allStrings().map((x) => x.s).join(' \n '));
  // Third legitimate origin: the wave briefs this file cites (the Schreiben
  // paragraph quotes PR D1's writing-topic list). Verified against the brief
  // itself, never allow-listed.
  const briefText = flat(
    [`${S}/wave4/exam-brief.md`, `${S}/wave4/test-brief.md`, `${S}/wave4/level-a2.1.md`]
      .map((f) => readFileSync(f, 'utf8'))
      .join(' \n ')
  );

  // Quotes that are neither: identifiers, English prose, and the level
  // file's own topic names. Anything German and sentence-shaped must resolve.
  const quotes = [...header.matchAll(/"([^"]{3,})"/g)].map((m) => m[1]);
  assert.ok(quotes.length >= 25, `expected the header to quote its evidence, found ${quotes.length}`);

  const unresolved = [];
  const seen = { source: 0, module: 0, brief: 0, meta: 0 };
  const META = new Set([
    // field names the header explains, quoted as names rather than as German
    'examKey / formatOf / level / title',
    'formatOf resolves to a real EXAM_TRACKS key',
    'am besten',
  ]);
  for (const q of quotes) {
    if (META.has(q)) {
      seen.meta += 1;
    } else if (dumpText.includes(q)) {
      seen.source += 1;
    } else if (moduleText.includes(q)) {
      seen.module += 1;
    } else if (briefText.includes(q)) {
      seen.brief += 1;
    } else {
      unresolved.push(q);
    }
  ...
  assert.deepEqual(
    unresolved,
    [],
    `header quotes in neither the source dump, this module, nor the wave briefs:\n${JSON.stringify(unresolved, null, 2)}`
  );
  assert.ok(seen.source >= 12, `expected the listening justification to rest on real quotes, got ${seen.source}`);
  assert.ok(seen.module >= 10, `expected the grammar inventory to quote real module strings, got ${seen.module}`);
});
...
test("the header's #5 count is the one the dump supports", () => {
  const dump = JSON.parse(readFileSync(`${S}/wave4/source/listening-a2.1.json`, 'utf8'));
  const ex5 = dump.find((e) => e.exercise_number === 5);
  const priceItems = ex5.questions.filter((q) => /kostet|kosten/.test(q.question_text));
  const numberItems = ex5.questions.filter((q) => /kostet|kosten|Wie viele|Welchen Lichtschutzfaktor/.test(q.question_text));
  assert.equal(priceItems.length, 5, 'the header claims five price items in #5');
  assert.equal(numberItems.length, 7, 'the header claims seven bare-number items in #5');
  assert.ok(!/7 of 10 items in #5 ask only for a price/.test(headerText()), 'round-1 miscount must be gone');
});
...
async function liveTasks() {
  const { WRITING_TASKS } = await import('/home/user/deutschmeister/src/data/writingTasks.js');
...
test('that meaning check actually fires on the round-3 Sportverein wording', async () => {
  ...
  const live = readFileSync('/home/user/deutschmeister/src/data/writingTasks.js', 'utf8');
