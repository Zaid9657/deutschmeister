...
| 2026-09-01 | — | — | — | — | Activation mailer FIRST real run (09:30 UTC): 1× activation_d1 (09:30:35), 1× activation_d4 (09:30:37), 0 failed — verified in `lifecycle_emails`. The gap vs the dry-check's "6+10" is NOT a bug: at run time 6 (d1) / 11 (d4) users were in-window with status `new`, but only 1 of each had `email_confirmed_at` set, and the mailer (correctly, same as trial-lifecycle) mails confirmed addresses only; the 6+10 estimate was a raw SQL count without that filter. Funnel insight: most stalled signups never confirm their email and are unreachable by ANY mailer — if activation volume matters, the lever is the confirmation step (resend-confirmation UX), not the mailer |
| 2026-09-02 | — | — | launch staged | — | Design renovation fully merged (789a479, all six waves). Mock exams authored for Goethe B1, DTZ and telc B2 on the Modelltest rails (PR #40) — every exam hub now demonstrates its Übungstest; DTZ result copy made scaled-exam-honest. Launch email 1 staged as ready-to-run script (`drafts/send-launch-email-1.sh`, test + live modes) on the send-campaign path — a Resend broadcast was rejected because the Resend audience is empty and the function already owns exclusions + HMAC unsubscribe. Still gating revenue (owner, ≤30 min total): finish START49 in LS (expiry 2026-09-14, restrict to product 1329566), one test-mode purchase (agent verifies DB), then run the send script test→live |
| 2026-09-02 (evening) | — | — | — | — | Activation lever shipped (PRs #41, #42 merged): (1) login rescue for unconfirmed accounts — auth logs caught a real user failing 9 logins with no way forward; (2) confirmation-nudge mailer for the 227 mailable unconfirmed signups, click-time magic link, one email ever per account — SHIPS OFF until owner applies migrations/2026-09-02-confirmation-nudge.sql + sets CONFIRM_NUDGE_ENABLED=true; (3) onboarding first-session fast lane. Funnel MEASURED (90d confirmed cohort): 307 confirmed → 271 (88%) finish onboarding → only 73 (24%) ever start a lesson, 57 of them on day 1 — activation is day-one-or-never; the fixed leaks were the false '5 min' placement-test promise (real: 15–20 min) and the 'Maybe later → dashboard' dead end. The 24% lesson-start rate is the number to watch weekly |
