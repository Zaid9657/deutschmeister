...
test('every sentence-building item with a time or place Angabe accepts both word orders — REVIEW #12 BLOCKER 1', () => {
  ...
  assert.deepEqual([...gradedIds].sort(), [
    ...
    // Round 24 (DaF review #23, Minor 1): the pool gained the Q letter-name item
    // (`extra-a11-l01-25`) and the L1 notice teaches `Ku`, which reseats all four papers. The
    // count rises 1 → 3 — the rule now FINDS two more graded items with a frontable Angabe
    // („Von Beruf bin ich…“ moved to cp1 seat 2, and two Bausteine word bags with a time Angabe
    // surfaced on cp3/cp4) — and every one of the three accepts both orders through the checker
    // (the loop above proves it before this pin is read). RE-MEASURED, not relaxed.
    'a1.1-cp1-schreiben-2',
    'a1.1-cp3-bausteine-4',
    'a1.1-cp4-bausteine-1',
