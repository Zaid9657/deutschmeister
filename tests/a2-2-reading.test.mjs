// Guard suite for the A2.2 reading rewrite (Course Factory Wave 5, PR C):
// migrations/2026-09-06-a2-2-reading.sql. Mirrors tests/a2-1-reading.test.mjs
// (Wave 4 PR C / #85); A2.2-specific: the two exam-format lessons are Goethe A2
// Lesen Teil 2 (Informationstafel, a/b/c) and Teil 4 (Anzeigen zuordnen — six ads
// a–f plus x, five situations, answers distinct with exactly one x), so the
// Teil-4 lesson may run to 190 words and its options are not a/b/c.
//
...
//      at level 'a2.2' (lowercase — reading_lessons' check constraint, unlike
...
const sql = readFileSync(join(ROOT, 'migrations', '2026-09-06-a2-2-reading.sql'), 'utf8');
...
test('the 2 new exam-format lessons are guarded INSERTs at level a2.2', () => {
  ...
  for (const stmt of inserts) {
    assert.match(stmt, /SELECT\s*\n\s*'a2\.2',/, 'insert must land at level a2.2 (lowercase)');
    assert.match(stmt, /WHERE NOT EXISTS \(\s*SELECT 1 FROM public\.reading_lessons WHERE level = 'a2\.2' AND title_de = '.*'\s*\);/);
  }
});
...
test('every rewrite carries 5 rf + 1 a/b/c choice, rf never all-one-answer, word_count <= 150', () => {
  ...
  for (const stmt of updates) {
    ...
    assert.ok(Number(wordCountMatch[1]) <= 150, `word_count ${wordCountMatch[1]} exceeds the 150-word A2.2 ceiling`);
  }
...
test('the exam-format lessons carry order_index 9 (Teil 2, a/b/c) and 10 (Teil 4, a–f + x)', () => {
  const inserts = statementsStartingWith(/^INSERT INTO public\.reading_lessons/gm, sqlNoComments);
  ...
  for (const [orderIndex, stmt] of byOrderIndex) {
    ...
    const answers = checks.map((c) => c.answer);
    if (orderIndex === 9) {
      for (const c of checks) {
        assert.deepEqual(c.options, ['a', 'b', 'c'], 'Teil-2 choice must offer exactly a/b/c');
        assert.ok(c.options.includes(c.answer), 'choice answer must be one of its options');
      }
      assert.deepEqual([...new Set(answers)].sort(), ['a', 'b', 'c'], 'Teil 2 must use all three keys');
    } else {
      for (const c of checks) {
        assert.deepEqual(c.options, ['a', 'b', 'c', 'd', 'e', 'f', 'x'], 'Teil-4 choice must offer the six ads plus x');
        assert.ok(c.options.includes(c.answer), 'choice answer must be one of its options');
      }
      assert.equal(new Set(answers).size, 5, 'Teil 4: five situations must map to five distinct answers');
      assert.equal(answers.filter((a) => a === 'x').length, 1, 'Teil 4: exactly one situation has no fitting ad');
    }
    const wordCountMatch = stmt.match(/,\s*(\d+),\s*\d+,\s*\d+\s*\nWHERE NOT EXISTS/);
    assert.ok(wordCountMatch, 'insert is missing word_count');
    const cap = orderIndex === 10 ? 190 : 150;
    assert.ok(Number(wordCountMatch[1]) <= cap, `word_count ${wordCountMatch[1]} exceeds the ${cap}-word ceiling for order_index ${orderIndex}`);
  }
});
...
test('ReadingChecks heads an all-choice lesson "Wähle a, b oder c" and keeps "Richtig oder falsch?" for mixed ones', () => {
  ...
  assert.match(jsx, /Wähle \$\{spell\(optionKeys, 'oder'\)\}/, 'German heading must spell a non-a/b/c option set (the Teil-4 lesson offers a–f and x)');
  assert.match(jsx, /Choose \$\{spell\(optionKeys, 'or'\)\}/, 'English heading must spell a non-a/b/c option set');
...
test('the a2 seed JSON mirrors the migration: 2 rewritten A2.2 lessons carry checks, 2 exam-format lessons appended', () => {
  const seed = JSON.parse(readFileSync(join(ROOT, 'content', 'reading', 'a2-lessons.json'), 'utf8'));
  const a22 = seed.filter((l) => String(l.level).toLowerCase() === 'a2.2');
  assert.equal(a22.length, 4, 'the a2 seed holds 2 rewritten + 2 exam-format A2.2 lessons');
  for (const l of a22) {
    assert.ok(Array.isArray(l.checks) && l.checks.length >= 5, `${l.title_de} is missing checks`);
    assert.ok(l.word_count <= (l.order_index === 10 ? 190 : 150), `${l.title_de} exceeds its word ceiling in the seed`);
  }
  assert.deepEqual(a22.map((l) => l.order_index), [7, 8, 9, 10]);
});
