// Guard suite for the four B1.1 topics added in Course Factory Wave 7 PR A
// (temporal-clauses, obwohl-damit-sodass, adverbial-connectors, two-part-connectors).
// Mirrors tests/a2-2-course.test.mjs (Wave 5 PR A / #93).
//
// What it pins, and why:
//
//   1. The four slugs exist at sub_level B1.1, topic_order 9-12, in the decided
//      order — the subordinator/connector set Wave 6 cut OUT of A2.2 that no B1.2/B2
//      slug owns: temporal-clauses and obwohl-damit-sodass (clause syntax the learner
//      knows from subordinate-word-order), then adverbial-connectors (main-clause
//      inversion), then two-part-connectors (mixes both) — see docs/course-factory-tracker.md
//      Decisions log, 2026-09-07 Wave 7 entry.
...
//   4. B1.1 now has exactly 12 topics, 1..12, no gaps or duplicates.
//   6. Every learner-facing German sentence of the four topics stays inside the
//      B1.1 sentence cap (20 words, docs/course-factory/wave7/level-b1.1.md).
...
const NEW_SLUGS = ['temporal-clauses', 'obwohl-damit-sodass', 'adverbial-connectors', 'two-part-connectors'];
const EXPECTED_ORDER = {
  'temporal-clauses': 9,
  'obwohl-damit-sodass': 10,
  'adverbial-connectors': 11,
  'two-part-connectors': 12,
};

test('the four new B1.1 topics exist at the expected topic_order, in the decided order', () => {
  for (const slug of NEW_SLUGS) {
    const topic = cache.topics.find((t) => t.slug === slug && t.sub_level === 'B1.1');
    assert.ok(topic, `topic "${slug}" not found at sub_level B1.1 in the cache`);
    assert.equal(topic.topic_order, EXPECTED_ORDER[slug], `topic "${slug}" has the wrong topic_order`);
  }
});

test('B1.1 now has exactly 12 topics, 1..12 with no gaps or duplicates', () => {
  const orders = cache.topics
    .filter((t) => t.sub_level === 'B1.1')
    .map((t) => t.topic_order)
    .sort((a, b) => a - b);
...
test('each new topic\'s Astro page title stays within 70 characters', () => {
  for (const slug of NEW_SLUGS) {
    const topic = cache.topics.find((t) => t.slug === slug && t.sub_level === 'B1.1');
    const title = `German ${topic.title_en} — B1.1 Grammar | DeutschMeister`;
...
test('the migration keeps the topic_order CHECK at 1..12', () => {
  const sql = readFileSync(join(ROOT, 'migrations', '2026-09-07-b1-1-new-topics.sql'), 'utf8');
...
test("the migration's guarded INSERT counts match the cache rows for the four new topics", () => {
  ...
  const topics = cache.topics.filter((t) => NEW_SLUGS.includes(t.slug) && t.sub_level === 'B1.1');
  assert.equal(topics.length, 4, 'expected exactly 4 new B1.1 topics in the cache');
...
test('every learner-facing German sentence of the four new topics is at most 20 words', () => {
  const words = (str) => str.trim().split(/\s+/).filter(Boolean).length;
  for (const slug of NEW_SLUGS) {
    const topic = cache.topics.find((t) => t.slug === slug && t.sub_level === 'B1.1');
    for (const e of cache.examples.filter((x) => x.topic_id === topic.id)) {
      assert.ok(words(e.sentence_de) <= 20, `${slug}: example ${e.order_index} has ${words(e.sentence_de)} words: "${e.sentence_de}"`);
    }
    for (const x of cache.exercises.filter((y) => y.topic_id === topic.id)) {
      assert.ok(words(x.question_de) <= 24, `${slug}: exercise (stage ${x.stage}, order ${x.order_index}) question_de has ${words(x.question_de)} words`);
      assert.ok(words(x.correct_answer) <= 20, `${slug}: exercise (stage ${x.stage}, order ${x.order_index}) correct_answer has ${words(x.correct_answer)} words`);
    }
  }
});
