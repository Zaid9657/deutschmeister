// Guard suite for the A2.2 listening additions (Course Factory Wave 5, PR C):
// migrations/2026-09-06-a2-2-listening.sql. Mirrors tests/a2-1-listening.test.mjs.
//
...
//   2. VOLUME — 78 new questions, 13 per exercise across the 6 A2.2
...
//      (18 rows).
//   5. NO EXISTING-ROW EDIT — the author's audit of the 60 live A2.2 questions
//      found nothing to fix, so the migration carries no UPDATE at all; a
//      stray UPDATE here would mean an unreviewed edit slipped in.
...
const sql = readFileSync(join(ROOT, 'migrations', '2026-09-06-a2-2-listening.sql'), 'utf8');
...
test('78 new questions, exactly 13 per exercise', () => {
  ...
  assert.equal(Object.keys(perExercise).length, 6, 'expected 6 distinct A2.2 exercises');
...
test('the migration carries no existing-row UPDATE (the live-question audit was clean)', () => {
  assert.equal(editUpdates.length, 0, `expected 0 existing-row UPDATEs, found ${editUpdates.length}`);
});
