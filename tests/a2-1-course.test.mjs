// Guard suite for the four A2.1 topics added in Course Factory Wave 4 PR A
// (adjective-endings-intro, pronouns-accusative-dative, modal-verbs-past, temporal-prepositions).
// Mirrors tests/a1-2-course.test.mjs (Wave 3 PR A / #74).
//
// What it pins, and why:
//
//   1. The four slugs exist at sub_level A2.1, topic_order 9-12, in the decided
//      order — adjective-endings-intro (needs dative-case + possessive-pronouns),
//      pronouns-accusative-dative (builds on the dative-pronouns table),
//      modal-verbs-past, temporal-prepositions (see docs/course-factory-tracker.md
//      Decisions log, 2026-09-05 Wave 4 entry).
...
//   4. A2.1 now has exactly 12 topics, 1..12, no gaps or duplicates.
...
const NEW_SLUGS = ['adjective-endings-intro', 'pronouns-accusative-dative', 'modal-verbs-past', 'temporal-prepositions'];
const EXPECTED_ORDER = {
  'adjective-endings-intro': 9,
  'pronouns-accusative-dative': 10,
  'modal-verbs-past': 11,
  'temporal-prepositions': 12,
};

test('the four new A2.1 topics exist at the expected topic_order, in the decided order', () => {
  for (const slug of NEW_SLUGS) {
    const topic = cache.topics.find((t) => t.slug === slug && t.sub_level === 'A2.1');
    assert.ok(topic, `topic "${slug}" not found at sub_level A2.1 in the cache`);
    assert.equal(topic.topic_order, EXPECTED_ORDER[slug], `topic "${slug}" has the wrong topic_order`);
  }
});

test('A2.1 now has exactly 12 topics, 1..12 with no gaps or duplicates', () => {
  const orders = cache.topics
    .filter((t) => t.sub_level === 'A2.1')
    .map((t) => t.topic_order)
    .sort((a, b) => a - b);
...
test('each new topic\'s Astro page title stays within 70 characters', () => {
  for (const slug of NEW_SLUGS) {
    const topic = cache.topics.find((t) => t.slug === slug && t.sub_level === 'A2.1');
    const title = `German ${topic.title_en} — A2.1 Grammar | DeutschMeister`;
...
test('the migration keeps the topic_order CHECK at 1..12', () => {
  const sql = readFileSync(join(ROOT, 'migrations', '2026-09-06-a2-1-new-topics.sql'), 'utf8');
...
test("the migration's guarded INSERT counts match the cache rows for the four new topics", () => {
  ...
  const topics = cache.topics.filter((t) => NEW_SLUGS.includes(t.slug) && t.sub_level === 'A2.1');
  assert.equal(topics.length, 4, 'expected exactly 4 new A2.1 topics in the cache');
