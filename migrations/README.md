...
| `2026-08-19-activation-emails.sql` | Widens the `lifecycle_emails` kind CHECK to allow `activation_day1` — the first lifecycle message selected on behaviour rather than a trial date (signed up yesterday, never opened a lesson; 136 of the last 152 signups qualify). Adds a `(kind, user_id)` index for the lookup the job runs every night. | ✅ **Applied 2026-08-19** |
